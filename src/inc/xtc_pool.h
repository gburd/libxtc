...
XTC_API int  xtc_pool_create(size_t capacity, xtc_pool_t **out);
...
XTC_API int  xtc_pool_checkout(xtc_pool_t *p, int64_t timeout_ns, void **out);
...
XTC_API int  xtc_pool_checkin(xtc_pool_t *p, void *resource);

/* Number of resources currently free (available for checkout). */
XTC_API size_t xtc_pool_available(const xtc_pool_t *p);

/* Total resources added to the pool (free + checked out). */
XTC_API size_t xtc_pool_capacity(const xtc_pool_t *p);
