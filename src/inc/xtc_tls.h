 ...
 *
 * Resolution, identical on every backend (OpenSSL, GnuTLS, wolfSSL,
 * mbedTLS, and since 1.51 SChannel): verify_peer_mode wins when not
 * DEFAULT; else a non-zero legacy verify_peer means REQUIRE; else the ROLE
 * default -- CLIENT: REQUIRE, SERVER: NONE.  (SChannel in 1.50 read only
 * verify_peer, so a zeroed SChannel CLIENT verified nothing.)
 *
