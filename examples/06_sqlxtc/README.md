...
* `bufmgr.c` -- a page-id-addressed buffer manager: a fix resolves a
  page id to a resident, pinned frame through a striped-lock page
  table, faulting the page in on a miss, with a cooling-stage CLOCK
  evictor.  A page-provider process (an `xtc_proc`) cools cold frames
  to COOL and writes the dirty ones out ahead of demand, so reclaiming
  a frame is a cheap state flip plus a page-table removal; page I/O is
  offloaded so the loop never stalls.  `test_bufmgr` cycles 200 pages
  through a 16-frame pool -- forcing cool -> write-out -> evict ->
  reload -- while the provider runs concurrently, and verifies every
  page survives.
