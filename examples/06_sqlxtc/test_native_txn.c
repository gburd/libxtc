...
static void
dump(sqlite3 *db, char *buf, size_t cap)
{
	sqlite3_stmt *s = NULL;
	size_t n = 0;
	buf[0] = 0;
	if (sqlite3_prepare_v2(db, "SELECT k, a FROM t ORDER BY k", -1, &s, 0) != SQLITE_OK)
		return;
...
static int
W(sqlite3 *db, const char *q)
{
	int64_t n = 0;
	return vx_run_write(db, q, &n, NULL);
}
...
main(void)
{
	...
	if (bm_create(&bo, &bm) != 0 || bt_open(bm, &bt) != 0 ||
	    ...
	    xstore_register(db, bt) != SQLITE_OK) {
		fprintf(stderr, "FAIL: setup\n");
		return 1;
	}
	sqlite3_exec(db, "CREATE VIRTUAL TABLE t USING xstore(k, a INT)", 0, 0, 0);

	/* Take native ownership of autocommit -- from here the native API
	 * drives the txn, NOT SQLite's autocommit flag. */
	CK(xstore_native_mode(db, 1) == 0, "native_mode on");
	...
	CK(xstore_native_begin(db) == 0, "native begin 2");
	CK(W(db, "INSERT INTO t(a) VALUES(99)") == 1, "in-txn insert to roll back");
	CK(xstore_rollback(db) == 0, "native rollback");
