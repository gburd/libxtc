...
# XTC_SIM_KEEP_BUILD=<dir> keeps the sim-backend build (and its .gcda
# coverage data) instead of using a throwaway mktemp dir that the trap
# removes.  Needed to MEASURE this tier's coverage: `check` and `check-dst`
# are separate configurations, so a make-check-only gcov run structurally
# undercounts everything only the sim backend reaches (evt/sim.c, and the
# sim-only branches of evt/exec.c and evt/loop.c).
if [ -n "${XTC_SIM_KEEP_BUILD:-}" ]; then
	work="$XTC_SIM_KEEP_BUILD"
	mkdir -p "$work"
else
	work="$(mktemp -d)"
...
for t in test_sim_sched test_sim_cross_loop_owner test_sim_sched_shares test_sim_pingpong test_sim_wake_park test_sim_fault test_sim_soak test_sim_critsec test_sim_latch test_sim_lockmgr test_sim_lwlock test_sim_lrlock test_sim_rcu test_sim_iofault test_sim_torn test_sim_stale test_sim_buggify test_sim_buggify2 test_sim_buggify3 test_sim_buggify4 test_sim_partition test_sim_machine_death test_sim_svr test_sim_fsm test_sim_credit test_sim_pool test_sim_pg test_sim_proc_teardown test_sim_xproc test_sim_chan test_sim_sync test_sim_reg test_sim_mctx test_sim_slab test_sim_pdict test_sim_chash test_sim_cskip test_sim_stats test_sim_tnt test_sim_exit_teardown test_sim_spawn_rel test_sim_determinism test_sim_launch test_sim_aiov test_sim_sup_strategy test_sim_app test_sim_blocking test_sim_osproc test_sim_res test_sim_stream test_sim_swarm test_sim_bufmgr test_sim_crash_recover test_sim_compose test_sim_compose_crash test_sim_composition test_sim_migratable test_sim_eager_rebalance test_sim_scope test_sim_trace_causal test_sim_dispatch test_sim_determinism_guard test_sim_saga test_sim_btree_merge test_sim_io_reg test_sim_aio_migrate; do
	...
	if [ "$t" = test_sim_bufmgr ]; then
		extra_obj="$bmobj"
		extra_inc="-I$bmdir"
	elif [ "$t" = test_sim_crash_recover ] || [ "$t" = test_sim_compose ] || [ "$t" = test_sim_compose_crash ] || [ "$t" = test_sim_btree_merge ]; then
		extra_obj="$engobjs"
