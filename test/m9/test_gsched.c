...
test_converge(const MunitParameter p[], void *d)
{
	...
	munit_assert_uint64(xtc_gsched_generation(g), >=, 100);

	xtc_gsched_destroy(g);
	return MUNIT_OK;
}
...
test_bounds(const MunitParameter p[], void *d)
{
	...
	for (eval = 0; eval < spec.population * 40; eval++) {
		...
		xtc_gsched_report(g, (double)genes[1]);
	}
	xtc_gsched_best(g, genes, NULL);
...
test_validate(const MunitParameter p[], void *d)
{
	...
	munit_assert_int(xtc_gsched_create(&spec, &g), ==, XTC_E_INVAL);
	spec.n_genes = 2; spec.population = 1;          /* too small */
	munit_assert_int(xtc_gsched_create(&spec, &g), ==, XTC_E_INVAL);
	spec.population = 8; spec.min[0] = 9; spec.max[0] = 1;  /* min>max */
	munit_assert_int(xtc_gsched_create(&spec, &g), ==, XTC_E_INVAL);
	munit_assert_int(xtc_gsched_create(NULL, &g), ==, XTC_E_INVAL);
	return MUNIT_OK;
}
...
static const MunitSuite suite = { "/m9/gsched", tests, NULL, 1, MUNIT_SUITE_OPTION_NONE };
